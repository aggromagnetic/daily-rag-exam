...
async function loadDashboardData() {
  ...
  try {
    ...
    // KSTOffset 보정을 통해 수험생 로컬 접속 기기 시간대 무관 KST 기준으로 날짜를 엄격히 동기화
    ...
    const curHour = todayKst.getUTCHours(); // KST 시간 (UTC+9의 시각이 getUTCHours로 바로 추출됨)

    const todayTests = tests.filter(t => t.date === todayStr);
    const hasAcc = todayTests.some(t => t.subject === '회계원리' && t.interactive);
    const hasFac = todayTests.some(t => t.subject === '시설개론' && t.interactive);
    const hasCiv = todayTests.some(t => t.subject === '민법' && t.interactive);

    const completedCount = [hasAcc, hasFac, hasCiv].filter(Boolean).length;
    const statusEl = document.getElementById('stat-today-exam-status');

    if (completedCount === 3) {
      statusEl.innerText = '출제 완료 🎉';
      statusEl.style.color = '#00f5a0';
    } else if (curHour < 6) {
      // 06시 이전: 추출 대기 상태
      statusEl.innerText = '추출 대기 중 ⏳ (06시 출제 시작)';
      statusEl.style.color = '#94a3b8';
    } else if (curHour >= 6 && curHour < 8) {
      // 06시~08시 사이: 추출 컴파일 기동 상태
      statusEl.innerText = `추출 중 ⚙️ (${completedCount}/3 완료)`;
      statusEl.style.color = '#3b82f6';
    } else {
      // 08시 이후인데 3개가 다 채워지지 못한 비정상 상태
      if (completedCount === 0) {
        statusEl.innerText = '출제 실패 ❌ (스케줄러 점검 요망)';
        statusEl.style.color = '#ff6b6b';
      } else {
        const failures = [];
        if (!hasAcc) failures.push('회계원리');
        if (!hasFac) failures.push('시설개론');
        if (!hasCiv) failures.push('민법');
        statusEl.innerText = `출제 완료 (일부 과목 실패: ${failures.join(', ')} 실패) ⚠️`;
      ...
      container.innerHTML = `
        <div class="empty-placeholder col-span-3">
          ...
          <p style="font-size: 0.75rem; margin-top: 0.5rem; color: var(--text-muted);">
            'npm run generate' 명령을 돌려 오늘의 모의고사를 즉시 출제받으시거나 아침 6시 자동 스케줄러를 가동해 주세요.
          </p>
        </div>
      `;
    }
  ...
  try {
    ...
    for (const item of history) {
      if (item.date === todayKst) {
        todayUsd += item.cost_usd || 0;
        todayKrw += item.cost_krw || 0;
      }
    }
