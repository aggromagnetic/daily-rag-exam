...
function parseItems(partText, isExplanation = false) {
  ...
  for (let line of lines) {
    ...
    if (match) {
      // 🚨 [해설 파서 업그레이드]: 해설 파트에서 맨 앞에 숫자가 오더라도 
      // '정답'이나 '해설' 키워드가 줄 내부에 아예 없다면 
      // 새로운 해설의 시작이 아닌 단순 계산 수식 줄로 판단하여 쪼개지 않고 기존 해설에 누적시킨다!
      let isRealExplanationStart = true;
      if (isExplanation) {
        const hasMarker = trimmed.includes('정답') || trimmed.includes('해설') || trimmed.includes('①') || trimmed.includes('②') || trimmed.includes('③');
        if (!hasMarker) {
...
async function main() {
  ...
  console.log(`🛠️ 5월 24일 회계원리 초정밀 2차 보정/교정 복원 기동...`);
  ...
  // 1번 문제는 고안한 문제로 덮어쓰고, 2번부터는 기존 문제 정렬
  ...
  for (let idx = 0; idx < explanations.length; idx++) {
    // 💡 기존 해설 항목 중 2번부터 35번까지를 순서대로 배치 (기존 파서의 0번 인덱스 해설은 2번 문제의 해설이었음)
    const newNum = idx + 2;
...
${finalExplanationsMarkdown.trim()}`;
  ...
  console.log(`✅ [V2 교정] 복원 덮어쓰기 성공! (최종 조립 결과 - 문제: 35개, 해설: 35개)`);
  
  const { execSync } = await import('child_process');
  try {
    execSync(`git add public/daily_tests/2026-05-24_회계원리.md`, { stdio: "inherit" });
    execSync('git commit -m "Completely repair Q1 and align explanation numbers in daily accounting exam"', { stdio: "inherit" });
    execSync("git push", { stdio: "inherit" });
    console.log("🎉 Git Push V2 최종 복구 배포 대성공!");
  } catch(e) {
