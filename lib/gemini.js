 ...
 */
function splitMarkdownIntoBatches(markdown, splitAtNumber = 21) {
  const headers = [
    "## [정답 및 상세 해설]",
    "### [정답 및 상세 해설]",
    "[정답 및 상세 해설]",
  ...
  const qSplitMatch = questionsPart.match(qSplitRegex);
  
  const eSplitRegex = new RegExp(`\\n${splitAtNumber}\\s*[\\.\\s번]`);
  const eSplitMatch = explanationsPart.match(eSplitRegex);
  
  if (!qSplitMatch || !eSplitMatch) {
    return [markdown];
  }
  
  const qPart1 = questionsPart.substring(0, qSplitMatch.index).trim();
  const qPart2 = questionsPart.substring(qSplitMatch.index).trim();
  
  const ePart1 = explanationsPart.substring(0, eSplitMatch.index).trim();
  const ePart2 = explanationsPart.substring(eSplitMatch.index).trim();
  
  const batch1 = `${qPart1}\n\n${selectedHeader}\n${ePart1}`;
  const batch2 = `## [시험 문제지] (제2부: ${splitAtNumber}번부터)\n\n${qPart2}\n\n${selectedHeader} (제2부: ${splitAtNumber}번부터)\n${ePart2}`;
...
function extractQuestionsAndExplanations(verifiedMarkdown) {
  ...
  const explanations = firstNewline !== -1 
...
async function verifySingleBatchWithGemini(rawBatchMarkdown, subjectName, docGuideName, batchLabel = "") {
  ...
  if (apiKeys.length === 0) {
    console.warn(`⚠️ [Gemini-Warning] 등록된 API Key가 없어 ${batchLabel} 검증을 건너뜁니다.`);
    return rawBatchMarkdown;
  }

  const systemRole = `당신은 대한민국 최고 권위의 주택관리사보 자격시험 수석 출제 검토관입니다. 
아래 전달받은 [초안 시험지]는 RAG가 1차 추출한 모의고사 본문(일부 배치)입니다. 오류 표기 교정, 잘못된 문제 번호 포맷(예: Q1., 문 1. 등), 오타, 지문 줄바꿈 훼손 등을 정밀 교정해 주십시오.
...
`;
  ...
  for (let i = 0; i < apiKeys.length; i++) {
    ...
    console.log(`📡 [Gemini-Verification] [${batchLabel}] [시도 ${i + 1}/${apiKeys.length}] API Key #${keyIdx + 1} (${keyMasked})로 요청을 전송 중...`);
    ...
    try {
      ...
      const minAcceptableLength = Math.max(1000, Math.floor(rawBatchMarkdown.length * 0.70));
...
export async function verifyQuizWithGemini(rawQuizMarkdown, subjectName, docGuideName) {
  console.log(`\n🪐 [Gemini-Verification] '${subjectName}' 과목의 2차 정밀 검증 및 분할 교정 루틴을 기동합니다...`);

  if (!rawQuizMarkdown || rawQuizMarkdown.trim().length < 200 || rawQuizMarkdown.includes('success": false')) {
    return rawQuizMarkdown;
  }

  const batches = splitMarkdownIntoBatches(rawQuizMarkdown, 21);

  if (batches.length === 1) {
    console.log("ℹ️ [Gemini-Verification] 문항 수가 적거나 분할 조건을 만족하지 않아 단일 배치로 검증을 가동합니다.");
    return await verifySingleBatchWithGemini(rawQuizMarkdown, subjectName, docGuideName, "단일 배치");
  }

  console.log(`⚡ [Gemini-Verification] 대형 문제지 감지! 총 ${batches.length}개 배치로 분할하여 로드 밸런싱 검증을 수행합니다.`);

  const verifiedBatches = [];
  for (let i = 0; i < batches.length; i++) {
    const batchLabel = `배치 ${i + 1}/${batches.length}`;
    console.log(`⏳ [Gemini-Verification] ${batchLabel} 검증 기동 중...`);
    const verified = await verifySingleBatchWithGemini(batches[i], subjectName, docGuideName, batchLabel);
    verifiedBatches.push(verified);
  ...
  console.log("🔄 [Gemini-Verification] 분할 검증 완료! 정교한 문제집 재조합(Merge) 루틴을 실행합니다...");
  try {
    const parts1 = extractQuestionsAndExplanations(verifiedBatches[0]);
    const parts2 = extractQuestionsAndExplanations(verifiedBatches[1]);
...
${parts2.explanations}`;
    ...
    console.log(`🎉 [Gemini-Verification] '${subjectName}' 과목 분할 검증 및 최종 재조합 완료! (총 크기: ${finalMarkdown.length}자)`);
    return finalMarkdown;
  } catch (error) {
    console.error("🚨 [Gemini-Verification] 재조합 중 오류 발생, 무결성 확보를 위해 RAG 1차 원본 초안으로 롤백합니다:", error.message);
    return rawQuizMarkdown;
  }
}
