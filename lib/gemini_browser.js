...
4. [🚨극히 중요] 제미나이의 내장 '대화형 퀴즈(Interactive Quiz)' 위젯이나 다른 내장 앱 위젯 기능을 호출하지 마십시오. 오직 일반 웹브라우저에서 직접 실행될 수 있는 단일 HTML 소스코드로만 작성하고, 마크다운 코드 블록(\`\`\`html ... \`\`\`) 내에 텍스트로 전부 출력해 주십시오. 설명이나 안내 문구는 생략하고 HTML 소스코드만 깔끔하게 출력해야 합니다.
...
`;
    ...
    while (elapsed < timeoutLimit) {
      ...
      if (currentHtml) {
        ...
        // [대화형 퀴즈 우회 자가치유 가동]
        // 만약 제미나이가 내장 대화형 퀴즈 위젯을 생성해서 코드 출력을 생략한 경우,
        // 우회 버튼("대화형 퀴즈 사용하지 않고 다시 시도하기")을 자동으로 클릭하여 소스코드로 재생성을 유도합니다.
        try {
          const fallbackClicked = await page.evaluate(() => {
            const els = Array.from(document.querySelectorAll('button, a, div[role="button"], span, p'));
            const btn = els.find(el => {
              const text = (el.innerText || el.textContent || '').trim();
              return text.includes('대화형 퀴즈 사용하지 않고');
            });
            if (btn) {
              btn.click();
              return true;
          ...
          if (fallbackClicked) {
            console.log("⚠️ [Gemini-Browser] 제미나이가 내장 대화형 퀴즈 위젯을 생성했습니다. '대화형 퀴즈 사용하지 않고 다시 시도하기' 버튼을 자동 클릭하여 HTML 코드로 재생성을 유도합니다...");
            // 전송 및 초기 재생성 시작을 위해 5초 대기
            await new Promise(resolve => setTimeout(resolve, 5000));
          }
        } catch (fbErr) {
          console.warn("⚠️ [Gemini-Browser] 자가치유 우회 버튼 클릭 시도 중 에러 발생:", fbErr.message);
        }
      }
    }
